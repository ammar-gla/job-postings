...
cat_names <- c("Lightcast","ONS estimated job adverts","Indeed")
pal_named <- setNames(object=pal,nm=cat_names)

comp_sources_chart <- ons_indeed_postings %>% 
  ...
  labs(title = paste0("Comparison of job postings data, London"),
       ...
       caption = paste0("Source: Lightcast (monthly online job postings, non-seasonally adjusted), Indeed (average of daily online job postings), ONS (average of weekly online job adverts, non-seasonally adjusted).","\n",
...
``` {r lfs_postings_chart, echo=FALSE,fig.cap = figcap_ons, out.width='100%'}
  ...
  # Save
  save_GLA_plot(chart_name)
  save_GLA_plot(chart_name,path=AD_HOC,w=10,h=6)
